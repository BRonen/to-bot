...
export const toReadKeywordRelations = relations(toReadKeywordSchema, ({ many }) => ({
    toReadsToKeywords: many(toReadsToKeywordsSchema),
}));

export const toReadsToKeywordsSchema = pgTable('to_reads_to_keywords', {
    toReadId: integer('to_read_id').notNull().references(() => toReadSchema.id),
    toReadKeywordId: integer('to_read_keywords_id').notNull().references(() => toReadKeywordSchema.id),
}, (t) => ({
    pk: primaryKey({ columns: [t.toReadId, t.toReadKeywordId] }),
}),
);

export const usersToGroupsRelations = relations(toReadsToKeywordsSchema, ({ one }) => ({
